...
//        options.pipeline = pipeline
      ...
      //imageView.sd_setImage(with: URL(string: photos[indexPath.row]), placeholderImage: UIImage(named: nil))

      
      //KingFisher
//        imageView.sd_setAnimationImages(with: [self.photos[indexPath.row]])
        let processor = ResizingImageProcessor(referenceSize: CGSize(width: 300, height: 300))
        imageView.kf.setImage(with: self.photos[indexPath.row], placeholder: nil, options: [.processor(processor)])
      //SDWebImage
